...
def content_based_filtering(product_id):
    ...
    recommend_id = []
    cate_level = ['canon', 'nikon', 'dslr', 'design', 'telephoto', 'color', 'stand', 'adapter', 'speedlite', 'hook',
                  'normal']
...
def index():
    ...
    # Close Connection
    cur.close()
    return render_template('home.html', camera=camera, lens=lens, ulens=ulens, acc=acc, form=form)
...
class RegisterForm(Form):
    ...
    password = PasswordField('', [validators.length(min=3)],
                             render_kw={'placeholder': 'Password'})
    mobile = StringField('', [validators.length(min=10, max=14)], render_kw={'placeholder': 'Mobile'})
...
def camera():
    ...
    if request.method == 'POST' and form.validate():
        ...
        return render_template('camera.html', camera=products, form=form)
    if 'view' in request.args:
        ...
        if 'uid' in session:
            ...
            if result:
                ...
                mysql.connection.commit()
        return render_template('view_product.html', x=x, cameras=product)
        ...
        product = curso.fetchall()
        x = content_based_filtering(product_id)
        return render_template('order_product.html', x=x, cameras=product, form=form)
    return render_template('camera.html', camera=products, form=form)
...
def lens():
    ...
    if request.method == 'POST' and form.validate():
        ...
        flash('Order successful', 'success')
        return render_template('lens.html', lens=products, form=form)
    if 'view' in request.args:
        q = request.args['view']
        product_id = q
        x = content_based_filtering(product_id)
        ...
        return render_template('view_product.html', x=x, cameras=products)
    elif 'order' in request.args:
        product_id = request.args['order']
        curso = mysql.connection.cursor()
        curso.execute("SELECT * FROM products WHERE id=%s", (product_id,))
...
@app.route('/ulens', methods=['GET', 'POST'])
def ulens():
    form = OrderForm(request.form)
    # Create cursor
    cur = mysql.connection.cursor()
    ...
    if request.method == 'POST' and form.validate():
        ...
        flash('Order successful', 'success')
        return render_template('ulens.html', ulens=products, form=form)
    if 'view' in request.args:
        q = request.args['view']
        product_id = q
        x = content_based_filtering(product_id)
...
def acc():
    ...
    if request.method == 'POST' and form.validate():
        ...
        flash('Order successful', 'success')
        return render_template('acc.html', acc=products, form=form)
    if 'view' in request.args:
        q = request.args['view']
        product_id = q
        x = content_based_filtering(product_id)
        ...
        if name and price and description and available and category and item and code and file:
            ...
            if picture.lower().endswith(('.png', '.jpg', '.jpeg')):
                ...
                if save_photo:
                    ...
                    if category == 'camera':
                        level = request.form.getlist('camera')
                        for lev in level:
                            yes = 'yes'
                        ...
                        level = request.form.getlist('lens')
                        for lev in level:
                            yes = 'yes'
                            query = 'UPDATE product_level SET {field}=%s WHERE product_id=%s'.format(field=lev)
                            curs.execute(query, (yes, product_id))
                        ...
                        level = request.form.getlist('ulens')
                        for lev in level:
                            yes = 'yes'
                            query = 'UPDATE product_level SET {field}=%s WHERE product_id=%s'.format(field=lev)
                            curs.execute(query, (yes, product_id))
                        ...
                        level = request.form.getlist('acc')
                        for lev in level:
                            yes = 'yes'
                            query = 'UPDATE product_level SET {field}=%s WHERE product_id=%s'.format(field=lev)
                            curs.execute(query, (yes, product_id))
...
class UpdateRegisterForm(Form):
    ...
    password = PasswordField('Password', [validators.length(min=3)],
                             render_kw={'placeholder': 'Password'})
    mobile = StringField('Mobile', [validators.length(min=10, max=14)], render_kw={'placeholder': 'Mobile'})
